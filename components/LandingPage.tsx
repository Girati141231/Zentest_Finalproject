        ...
        <div className="min-h-screen bg-[#050505] text-white flex flex-col font-sans selection:bg-blue-500/30 overflow-hidden relative">
            ...
            <header className="px-8 py-6 flex items-center justify-between z-10">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold tracking-tighter text-xl">ZENTEST</span>
                </div>
                ...
                <div className="flex items-center gap-6">
                    ...
                    {user ? (
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 pr-4 border-r border-white/10">
                                <div className="text-right hidden sm:block">
                                    <div className="text-[10px] text-white/40 font-bold uppercase tracking-widest">Logged in as</div>
                                    <div className="text-xs font-bold">{user.displayName || 'Guest User'}</div>
                                </div>
                                <img
                        ...
                        <button
                            onClick={onLogin}
                            className="bg-white/5 border border-white/10 text-white px-5 py-2 rounded-full text-xs font-bold hover:bg-white hover:text-black transition-all"
                        >
                            Sign In
                        </button>
                    )}
            ...
            <main className="flex-1 flex flex-col items-center justify-center text-center px-6 relative z-10 mt-[-40px]">
                {!user && (
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold uppercase tracking-widest text-blue-400 mb-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
                        <span className="w-1.5 h-1.5 rounded-full bg-blue-400 animate-pulse" /> v2.4 Now Available
                    </div>
                )}

                <h1 className="text-5xl md:text-7xl font-black tracking-tight mb-6 max-w-4xl leading-tight animate-in fade-in slide-in-from-bottom-8 duration-1000">
                    {user ? (
                        <>
                            Welcome Back, <br />
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300">
                    {user ? (
                        <button
                            ...
                            className="h-14 px-8 bg-white text-black font-bold rounded-sm hover:bg-blue-50 transition-all flex items-center gap-3 shadow-[0_0_30px_rgba(255,255,255,0.2)] hover:shadow-[0_0_50px_rgba(255,255,255,0.5)] active:scale-95 group text-lg"
                        >
                        ...
                        <>
                            <button
                                onClick={onLogin}
                                className="h-12 px-8 bg-white text-black font-bold rounded-sm hover:bg-blue-50 transition-all flex items-center gap-2 shadow-[0_0_30px_rgba(255,255,255,0.2)] hover:shadow-[0_0_40px_rgba(255,255,255,0.4)] active:scale-95 group"
                            >
                            ...
                            <button
                                onClick={onDemo}
                                className="h-12 px-8 bg-white/5 border border-white/10 text-white font-bold rounded-sm hover:bg-white/10 transition-all flex items-center gap-2 active:scale-95"
                            >
